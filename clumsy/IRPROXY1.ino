...
void loop()
{
  ...
  nTempVal = analogRead(LeftSensor);

  
  Serial.print("Readling Left Sensor ");
  Serial.println(nTempVal);

  if (nTempVal < L_sensor_val)
  
      Serial.print("Goes Down: Prev = ");
  else
      Serial.print("\t\t\t\t\tGoes Up: Prev = ");    
    
      Serial.print(L_sensor_val);
      Serial.print("\t New = ");
      Serial.println(nTempVal);
  L_sensor_val = nTempVal;
  delay(1000);
